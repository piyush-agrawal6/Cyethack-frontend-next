  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-[1312px] mx-auto px-6 text-center">
        {/* Animated Tag */}
        <span
          className={`inline-flex items-center gap-2 mb-4 px-4 py-1 text-sm font-medium rounded-full bg-cyan-700/30 text-cyan-400 border border-cyan-500/50 ${isVisible ? "animate-fade-in-down" : "opacity-0"
            }`}
        >
          <ShieldCheck className="w-4 h-4" /> Know What You Protect
        </span>
        ...
        >
          Complete Asset <br className="hidden md:block" />
        ...
        >
          Cyethack Solutions eliminates blind spots by discovering, classifying, and
          monitoring every asset in your IT and OT environment, ensuring nothing goes unprotected.
        </p>
  ...
  const challenges = [
    {
      title: "Shadow IT",
      description:
        "Unapproved devices and applications deployed without IT knowledge create massive security gaps and compliance risks.",
      icon: <Eye className="w-8 h-8 text-red-500" />,
    },
    {
      title: "Unmanaged Devices",
      description:
        "IoT and OT devices often lack agents and are invisible to traditional scanning tools, leaving them vulnerable to exploitation.",
      icon: <MonitorCheck className="w-8 h-8 text-orange-500" />,
    },
    {
      title: "Outdated Inventory",
      description:
        "Manual spreadsheets and static CMDBs are obsolete the moment they are created, leading to poor decision-making during incident response.",
...
const assetServices = [
  {
    ...
    description:
      "Continuous, passive scanning of your network to identify all connected devices (IT, OT, IoT) without disrupting operations.",
    icon: <Search className="w-6 h-6 text-cyan-600" />,
    image: "/services/asset_discovery.png",
  },
  {
    title: "Vulnerability Correlation",
    description:
      "Map discovered assets to known vulnerabilities (CVEs) and prioritize remediation based on asset criticality and exposure.",
    icon: <AlertTriangle className="w-6 h-6 text-cyan-600" />,
    image: "/services/asset_vuln.png",
  },
  {
    title: "Software Bill of Materials (SBOM)",
    description:
      "Generate detailed inventories of software components and dependencies to manage supply chain risks and license compliance.",
    icon: <FileText className="w-6 h-6 text-cyan-600" />,
    image: "/services/asset_sbom.png",
  },
  {
    title: "CMDB Enrichment",
    description:
      "Automatically update your Configuration Management Database (CMDB) with real-time asset data, ensuring a single source of truth.",
    icon: <Database className="w-6 h-6 text-cyan-600" />,
    image: "/services/asset_cmdb.png",
  },
];
...
const OTSecurityServicesSection = () => {
  ...
  const currentService = assetServices[activeService];

  return (
    <section id="asset-services" className="py-32 bg-white" ref={ref}>
      <div className="max-w-[1312px] mx-auto px-6">
        <div className="mb-20">
          <p
            className={`text-sm font-bold text-cyan-600 uppercase tracking-wider mb-2 ${isVisible ? "animate-fade-in delay-100" : "opacity-0"
              }`}
          >
            Key Services
          </p>
          ...
          <div
            className={`h-1 w-16 bg-cyan-500 rounded-full mb-8 ${isVisible ? "animate-fade-in delay-300" : "opacity-0"
              }`}
          />
          <p
          ...
          >
            You can't secure what you can't see. Our services provide a comprehensive,
            real-time view of your entire digital estate, empowering you to manage risk effectively.
          </p>
        </div>
          ...
          <div className="lg:w-1/2 w-full sticky lg:top-28">
            ...
            >
              <img
                ...
                alt={`Asset Service: ${currentService.title}`}
...
// --- MAIN EXPORT ---
// --- MAIN EXPORT ---
export default function InventoryAssetManagementPage() {
  return (
    <div className="min-h-screen bg-white">
      <HeroSection />
      <TheChallengeSection />
      <OurSolutionSection /> {/* New Methodology Section */}
      <OTSecurityServicesSection /> {/* Renamed/Reused for Asset Services */}
